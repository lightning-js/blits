...
test('Uses optional chaining in injected guards to protect against null/undefined access', (assert) => {
  const input = `
    import Blits from '@lightningjs/blits'
    export default Blits.Component('C', {
      computed: {
  ...
  assert.ok(result, 'injects guards')
  const out = result.code
  assert.ok(out.includes('this.media?.poster?.src;'), 'uses optional chaining in injected guard')
  assert.end()
})
